...
copy_properties(Arbiter, {
    ...
    inform: function(j, c, b) {
        ...
        for (var e = 0; e < l.length; e++) {
            ...
            window.ArbiterMonitor && ArbiterMonitor.log("event", j, c, a);
            var h;
            if (a._index[j]) {
                var k = $A(a._index[j]);
            ...
            window.ArbiterMonitor && ArbiterMonitor.log("done", j, c, a);
            i[j] = h;
        }
        return m ? i : i[l[0]];
    },
...
(function() {
    ...
    var j = m + o + "_", g = (new Date(+(new Date) + 6048e5)).toGMTString(), e = window.location.hostname.replace(/^.*(facebook\..*)$/i, "$1"), f = "; expires=" + g + ";path=/; domain=" + e, d = 0, p = false, k, n = false, h = document.cookie.length, i = false, l = +(new Date);
    function c(q) {
...
var user_action = function() {
    var l = !window.ArbiterMonitor ? "r" : "a", n = 0, m, e, f, o = 0, k, i, b, c, h = [ 0, 0, 0, 0 ], d = function() {
        ...
        if (window.collect_data_attribs) {
            p = collect_data_attribs(i, [ "ft", "gt" ]);
            copy_properties(p.ft, c.ft || {});
            copy_properties(p.gt, c.gt || {});
        }
        if (l == "a") {
            ArbiterMonitor.initUE(q);
            f = ArbiterMonitor.getInternRef(i);
        }
        window.EagleEye && EagleEye.log("act", [ o, n, e || "-", b, m || "-", f || d(i), l, window.URI ? URI.getRequestURI(true, true).getUnqualifiedURI().toString() : location.pathname + location.search + location.hash, p ].concat(h));
        k = true;
...
Function.prototype.extend = function() {
    return Class.extend.apply(Class, [ this ].concat([].slice.call(arguments, 0)));
};

Function.prototype.mixin = function() {
    return Class.mixin.apply(Class, [ this ].concat([].slice.call(arguments, 0)));
};
...
(function() {
    ...
    for (var b = 0; b < c.length; b++) JSLogger.prototype[c[b]] = function(event, d) {
        if (a.backlog.length < 100) a.backlog.push([ this.cat, c[b], event, a.stringify(d) ]);
    };
})();
