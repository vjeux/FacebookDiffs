...
copy_properties(KeyEventController.prototype, {
    mapKey: function(a) {
        if (a >= 0 && a <= 9) {
            if (typeof a != "number") a = a.charCodeAt(0) - 48;
            return [ 48 + a, 96 + a ];
        }
...
copy_properties(PhotoSnowboxLog, {
    ...
    logPhotoViews: function(a) {
        if (this.views) {
            ...
            (new AsyncRequest).setURI("/ajax/photos/snowbox/session_logging.php").setAllowCrossPageTransition(true).setOption("asynchronous", a != PhotoSnowboxLog.UNLOAD).setOption("suppressErrorHandlerWarning", true).setData(b).send();
            this.views = 0;
            this.fbidList = [];
            this.first = false;
            if (this.last) {
...
var PhotoSnowbox = {
    ...
    getThumbAndSize: function(b) {
        ...
        if (!a.x || !a.y) return;
        this.normalSize = a;
        if (!CSS.hasClass(b, "uiMediaThumb") && !CSS.hasClass(b, "uiPhotoThumb") && !CSS.hasClass(b, "uiScaledThumb")) return;
    ...
    initializeNodes: function(a) {
        ...
        this.actionList = $("fbPhotoSnowboxActions");
        this.sideAdUnit = $("fbPhotoSnowboxAdsSide");
        this.bottomAdUnit = $("fbPhotoSnowboxAdsBottom");
    },
    getRoot: function() {
        return this.root;
    },
    ...
    _open: function(d, c) {
        this.createLoader(c);
        CSS.show(this.root);
        this.deferUntilRepaint(CSS.addClass.curry(document.documentElement, "theaterMode"));
        ...
        this.stageHandlers = [ Event.listen(window, "resize", this.adjustForResize.bind(this)), Event.listen(this.root, "click", this.closeListener.bind(this)), Event.listen(this.stageWrapper, "click", this.buttonListener.bind(this)), Event.listen(this.actionList, "click", this.rotateListener.bind(this)) ];
    ...
    closeCleanup: function() {
        ...
        this.recacheData();
        DOM.empty(this.sideAdUnit);
        DOM.empty(this.bottomAdUnit);
        this.stream.destroy();
    ...
    adjustStageSize: function(c) {
        ...
        CSS.conditionClass(this.container, "adsOnSide", this.currentMinSize.x >= PhotoSnowbox.WIDE_ADS);
        this.adjustForAdUnit();
    ...
    loadAds: function() {
        var a = +(new Date);
        if (a - this.lastAdsLoad > PhotoSnowbox.ADS_REFRESH_RATE) {
            UIPagelet.loadFromEndpoint("WebEgoPane", "fbPhotoSnowboxAdsSide", {
                pid: 34,
                data: []
            }, {
                crossPage: true
    ...
    adjustForAdUnit: function() {
        var b = this.sideAdUnit.childNodes;
        var a = this.bottomAdUnit.childNodes;
        if (CSS.hasClass(this.container, "adsOnSide")) {
            if (a.length > 0 && b.length === 0) DOM.setContent(this.sideAdUnit, $A(a));
            DOM.empty(this.bottomAdUnit);
        } else if (b.length > 0) {
            DOM.setContent(this.bottomAdUnit, $A(b));
            DOM.empty(this.sideAdUnit);
        }
    },
    ...
    storeFromResponse: function(a) {
        window.ArbiterMonitor && ArbiterMonitor.stopTtiMeasurement();
